#!/bin/bash
# =============================================================================
# Stripe Connect Integrity Test — Setup & Execution
# =============================================================================
...
echo "============================================================"
echo "  STRIPE CONNECT INTEGRITY TEST"
echo "============================================================"
...
CONNECTED_ACCT_RESPONSE=$(curl -s -X POST https://api.stripe.com/v1/accounts \
  ...
  -d "type=custom" \
  -d "country=BR" \
  -d "email=k6-integrity-test@fellowcore.io" \
  ...
  -d "individual[last_name]=Test" \
  -d "individual[email]=k6-test@fellowcore.io" \
...
dotnet run --project src/FellowPay.Api --no-launch-profile --no-build -c Release > /tmp/fellowcore-api.log 2>&1 &
API_PID=$!
...
for i in $(seq 1 60); do
  ...
  if ! kill -0 "$API_PID" 2>/dev/null; then
    ...
    tail -30 /tmp/fellowcore-api.log
    exit 1
  fi
  sleep 1
done
...
echo ""
echo "============================================================"
echo "  RUNNING K6 INTEGRITY TEST"
echo "============================================================"
echo ""

mkdir -p "$PROJECT_DIR/tests/results"

k6 run "$PROJECT_DIR/tests/k6/stripe-connect-integrity.js" \
  -e BASE_URL="$BASE_URL" \
  -e API_KEY="$API_KEY" \
...
# ─── Step 8: Show Report ───────────────────────────────────────────
echo ""
REPORT_FILE="$PROJECT_DIR/tests/results/stripe-integrity-report.json"
