#!/bin/bash
# =============================================================================
# Multi-Method Payment Integrity Test — Setup & Execution
# =============================================================================
# 1. Creates a Stripe Connected Account (sandbox)
# 2. Links it to the seeded seller in PostgreSQL
# 3. Configures tenant for multi-method support (PIX via Stripe)
# 4. Starts the API with webhook secret configured
# 5. Runs the k6 multi-method integrity test
# 6. Cleans up
# =============================================================================
...
WEBHOOK_SECRET="${WEBHOOK_SECRET:-whsec_k6_multi_method_test_secret_2024}"
API_KEY="${API_KEY:?Set API_KEY env var}"
BASE_URL="http://localhost:5195"

API_PID=""
ORIGINAL_PIX_PROVIDER=""
...
cleanup() {
  ...
  # Restore PIX provider setting
  if [ -n "$ORIGINAL_PIX_PROVIDER" ]; then
    echo "Restoring ActivePixProvider to $ORIGINAL_PIX_PROVIDER..."
    run_psql "UPDATE \"TenantConfigs\" SET \"ActivePixProvider\" = $ORIGINAL_PIX_PROVIDER;" || true
  fi
}
trap cleanup EXIT
...
echo "============================================================"
echo "  MULTI-METHOD PAYMENT INTEGRITY TEST"
echo "============================================================"
...
SELLER_ID_DB=$(run_psql "SELECT \"Id\" FROM \"Sellers\" LIMIT 1;" | tr -d ' \n')
if [ -z "$SELLER_ID_DB" ]; then
  echo "ERROR: No seller found in database. Run the API once to seed data."
  exit 1
fi
echo "Seller ID: $SELLER_ID_DB"

# ─── Step 3: Configure PIX via Stripe ──────────────────────────────
echo ""
echo "--- Configuring PIX provider to STRIPE for multi-method test ---"
# PaymentProvider enum: STRIPE=0, OPENPIX=1, SANDBOX=2
ORIGINAL_PIX_PROVIDER=$(run_psql "SELECT \"ActivePixProvider\" FROM \"TenantConfigs\" LIMIT 1;" | tr -d ' \n')
echo "Original ActivePixProvider: $ORIGINAL_PIX_PROVIDER"
run_psql "UPDATE \"TenantConfigs\" SET \"ActivePixProvider\" = 0;"
echo "ActivePixProvider set to STRIPE (0)"

# ─── Step 4: Create Stripe Connected Account ──────────────────────
echo ""
...
CONNECTED_ACCT_RESPONSE=$(curl -s -X POST https://api.stripe.com/v1/accounts \
  ...
  -d "type=custom" \
  -d "country=BR" \
  -d "email=k6-multi-method-test@fellowcore.io" \
  ...
  -d "individual[last_name]=MultiMethod" \
  -d "individual[email]=k6-multi@fellowcore.io" \
...
if [ -z "$CONNECTED_ACCT" ]; then
  ...
  # ─── Step 5: Link Connected Account to Seller ──────────────────
  echo ""
  echo "--- Linking Connected Account to Seller ---"
  run_psql "UPDATE \"Sellers\" SET \"ExternalAccountId\" = '${CONNECTED_ACCT}' WHERE \"Id\" = '${SELLER_ID_DB}';"
...
dotnet run --project src/FellowPay.Api --no-launch-profile --no-build -c Release > /tmp/fellowcore-api-multi-method.log 2>&1 &
API_PID=$!
...
for i in $(seq 1 60); do
  ...
  if ! kill -0 "$API_PID" 2>/dev/null; then
    ...
    tail -30 /tmp/fellowcore-api-multi-method.log
    exit 1
  fi
  sleep 1
done
...
echo ""
echo "============================================================"
echo "  RUNNING MULTI-METHOD INTEGRITY TEST"
echo "============================================================"
echo ""

mkdir -p "$PROJECT_DIR/tests/results"

k6 run "$PROJECT_DIR/tests/k6/multi-method-integrity.js" \
  -e BASE_URL="$BASE_URL" \
  -e API_KEY="$API_KEY" \
  -e STRIPE_SK="$STRIPE_SK" \
  -e WEBHOOK_SECRET="$WEBHOOK_SECRET" \
  -e SELLER_ID="$SELLER_ID_DB"

# ─── Step 8: Restore Seller ────────────────────────────────────────
if [ -n "$CONNECTED_ACCT" ]; then
  echo ""
  echo "--- Restoring seller (removing test connected account) ---"
  run_psql "UPDATE \"Sellers\" SET \"ExternalAccountId\" = NULL WHERE \"Id\" = '${SELLER_ID_DB}';" || true
fi

# ─── Step 9: Show Report ───────────────────────────────────────────
echo ""
REPORT_FILE="$PROJECT_DIR/tests/results/multi-method-report.json"
